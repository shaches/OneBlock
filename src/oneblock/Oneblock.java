...
public class Oneblock extends JavaPlugin {
   ...
   * atomically-replaceable unit. Writers mutate the origin via {@link #setPosition} / {@link
   * #setOffset} which perform one atomic swap each; readers call {@link #origin()} once and use the
   * captured snapshot so that a concurrent {@code /ob set} cannot produce a mixed (torn) view (e.g.
   * new {@code x} with old {@code offset}).
   *
   * <p>Package-private so test code can reflect it; callers outside the package use the getter
   * helpers ({@link #getX()}, {@link #getOffset()}, ...) or {@link #origin()}.
   */
  static final AtomicReference<IslandOrigin> ORIGIN = new AtomicReference<>(IslandOrigin.EMPTY);

  /**
   * Singleton holder for every admin-toggleable flag the plugin owns. Phase 3 extracted what used
   * to be ≈17 {@code public static volatile} fields on this class into {@link Settings} (a typed
   * bag of {@code volatile} instance fields). Callers reach the live values through {@link
   * #settings()} — e.g. {@code Oneblock.settings().particle} — which preserves the per-field
   * cross-thread visibility we already had while letting tests instantiate a fresh {@code Settings}
   * without spinning up Bukkit.
   */
  ...
  /**
   * Leave-world reference. Written only by the main thread ( {@link Initialization} task's first
   * read picks up a freshly-assigned value. Separate from {@link #ORIGIN} because it belongs to a
   * different workflow (player teleport destination) and is updated independently.
   */
  ...
  /**
   * True after the four steady-state runners ({@code PlayerCacheRefresh}, {@code PlayerDataSave},
   * {@code IslandParticle}, {@code IslandBlockGen}) have been scheduled by {@link #runMainTask()}.
   * Read by {@code JoinCommand} to decide whether the first {@code /ob j} after a fresh server
   * start needs to kick off the runners. Public for cross-package access from {@code
   * oneblock.command.sub.*} after the Phase 3.5 split.
   */
  ...
  /**
   * Shorthand for {@code origin().world()}. May be {@code null} before the configured world
   * finishes loading; callers must guard.
   */
  public static World getWorld() {
    return ORIGIN.get().world();
  }

  /** Snapshot of the current island origin. Always non-null. */
  public static IslandOrigin origin() {
    return ORIGIN.get();
  }
  ...
  public int findNearestRegionId(final Location loc) {
    return IslandCoordinateCalculator.findNearestRegionId(loc);
  }
  ...
  private void setupMetrics(Metrics metrics) {
    ...
    metrics.addCustomChart(
        new SimplePie("use_empty_islands", () -> String.valueOf(SETTINGS.useEmptyIslands)));
  ...
  public void runMainTask() {
    ...
    if (getOffset() == 0) return;
    Bukkit.getScheduler()
        .runTaskTimerAsynchronously(this, new PlayerCacheRefreshTask(this), 0, 120);
    Bukkit.getScheduler().runTaskTimerAsynchronously(this, new PlayerDataSaveTask(this), 200, 6000);
    if (!Compat.superlegacy)
      Bukkit.getScheduler().runTaskTimerAsynchronously(this, new IslandParticleTask(this), 40, 40);
  ...
  public void reloadBorders() {
    ...
    if (w == null) return;
    if (SETTINGS.border)
      w.getPlayers().forEach(pl -> plugin.updateBorderLocation(pl, pl.getLocation()));
  ...
  private void loadPlayerData() {
    ...
    if (!PlayerInfo.list.isEmpty()) {
      getLogger()
          .info(
              "Player data has been successfully obtained from the "
                  + DatabaseManager.dbType
                  + " database.");
      return;
    }

    if (JsonPlayerDataStore.f.exists()) PlayerInfo.replaceAll(JsonPlayerDataStore.read());
    else PlayerInfo.replaceAll(LegacyYamlPlayerDataStore.read());
  }

  public void setPosition(Location loc) {
    setPosition(loc.getWorld(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ());
  }
  ...
  public void setPosition(World world, int x, int y, int z) {
    ...
    IslandOrigin next =
        ORIGIN.updateAndGet(
            prev -> new IslandOrigin(world != null ? world : prev.world(), x, y, z, prev.offset()));
  ...
  /**
   * Atomically set the cell-edge length ({@code offset}) that drives the island grid. Also persists
   * the new value to {@code config.yml} under the key {@code set}. Safe to call only from the main
   * thread (admin {@code /ob set}) — config writes assume main-thread ownership.
   */
  ...
  /**
   * Atomically replace the world component of {@link #ORIGIN} while preserving the loaded {@code
   * x/y/z/offset}. Used by the async {@code WorldInitTask} when a configured world becomes
   * available after a delayed Bukkit load. The swap is one CAS operation, so a concurrent {@code
   * /ob set} on the main thread either fully wins or is fully overwritten - it cannot produce a
   * torn snapshot.
   */
  ...
  public Location getLeave() {
    return new Location(
        leavewor,
        config.getDouble("xleave"),
        config.getDouble("yleave"),
        config.getDouble("zleave"),
        (float) config.getDouble("yawleave"),
        0f);
  }

  public void setLeave(Location loc) {
    setLeave(loc.getWorld(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(), loc.getYaw());
  }
  ...
  public static int countVisitors(UUID playerUuid) {
    ...
    if (regionId != -1)
      for (Player ponl : plugin.cache.getPlayers())
        if (plugin.findNearestRegionId(ponl.getLocation()) == regionId) count++;
    return count;
  }
  ...
  public static int getTopPosition(PlayerInfo player) {
    ...
    for (int i = 0; i < sorted.size(); i++) {
      ...
      if (entry != null && player.uuid.equals(entry.uuid)) return i;
    }
