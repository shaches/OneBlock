...
public class GUI {
  ...
  public static void openGUI(Player p) {
    ...
    if (p == null) return;
    Inventory mainGUI =
        Bukkit.createInventory(new GUIHolder(GUIHolder.GUIType.MAIN_MENU), 18, Messages.baseGUI);
    ...
    if (p.hasPermission("Oneblock.idreset"))
      mainGUI.setItem(
          8, setMeta(XMaterial.BARRIER, ChatColor.RED + "/ob idreset", Messages.idresetGUI));
  ...
  public static void acceptGUI(Player p, String name) {
    ...
    if (p == null) return;
    Inventory acceptGUI =
        Bukkit.createInventory(new GUIHolder(GUIHolder.GUIType.INVITE), 9, Messages.acceptGUI);
    acceptGUI.setItem(6, setMeta(XMaterial.REDSTONE_BLOCK, Messages.acceptGUIignore));
    acceptGUI.setItem(
        2,
        setMeta(
            XMaterial.EMERALD_BLOCK,
            String.format(Messages.acceptGUIjoin, name),
            Messages.idresetGUI));
  ...
  public static void visitGUI(Player p, OfflinePlayer[] offlinePlayers) {
    ...
    if (p == null) return;
    Inventory visitGUI =
        Bukkit.createInventory(new GUIHolder(GUIHolder.GUIType.VISIT), 54, Messages.visitGUI);
