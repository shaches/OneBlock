...
public final class CommandHandler implements CommandExecutor {
  ...
  /**
   * Phase 3.5a router registry. Each {@link Subcommand} is registered under its primary {@link
   * Subcommand#name()}; aliases (e.g. {@code j} -> {@code join}) reuse the same instance under the
   * alias key. Phase 3.5a leaves this map empty so every dispatch still falls through to the legacy
   * {@code switch} below; the next sub-slice (3.5b) populates it as subcommands are extracted into
   * {@code oneblock.command.sub.*}. Once the registry covers every {@code args[0]}, the legacy
   * switch shrinks to just the version-info fallback.
   */
  ...
  public static boolean idresetCommand(OfflinePlayer pl) {
    ...
    if (!Oneblock.settings().savePlayerInventory && pl instanceof Player)
      ((Player) pl).getInventory().clear();

    if (OBWorldGuard.isEnabled()) Oneblock.plugin.worldGuard.removeMember(uuid, PlId);
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    sender.sendMessage(
        ChatColor.values()[Oneblock.rnd.nextInt(ChatColor.values().length)]
            ...
            + "\n▀▄▀ █▄█  by MrMarL"
            + "\nPlugin version: v"
            + Oneblock.plugin.version
            + "\nServer version: "
            + (Compat.superlegacy ? "super legacy " : (Compat.legacy ? "legacy " : ""))
