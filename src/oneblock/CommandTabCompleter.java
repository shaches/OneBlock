...
public final class CommandTabCompleter implements TabCompleter {
  private final List<String> BASE_COMMANDS =
      Arrays.asList("j", "join", "leave", "invite", "accept", "kick", "help", "gui", "top");
  private final List<String> VISIT_COMMANDS = Arrays.asList("v", "visit");
  private final List<String> ADMIN_COMMANDS =
      Arrays.asList(
          "set",
          "setleave",
          "progress_bar",
  ...
  private void addOnlinePlayers(List<String> completions) {
    for (Player ponl : Oneblock.plugin.cache.getPlayers()) completions.add(ponl.getName());
  }
}
