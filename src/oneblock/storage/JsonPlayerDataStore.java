...
/**
 * JSON-backed player data store. Writes / reads the {@code PlData.json} file in the plugin data
...
public class JsonPlayerDataStore {
  ...
  public static void write(List<PlayerInfo> pls) {
    ...
    for (int i = 0; pls.size() > i; i++) {
      ...
      for (UUID us : pl.uuids) arr.add(us.toString());
  ...
  public static List<PlayerInfo> read() {
    ...
    try (InputStreamReader reader =
      ...
      Oneblock.plugin
          .getLogger()
          .warning("Failed to read player data from JSON: " + e.getMessage());
    }

    List<PlayerInfo> infs = new ArrayList<PlayerInfo>();
    if (main == null) return infs;
    PlayerInfo nullable = new PlayerInfo(null);
    Object idObj = main.get("id");
    if (!(idObj instanceof Number)) {
      Oneblock.plugin
          .getLogger()
          .warning("[Oneblock] PlData.json missing or non-numeric 'id' header; treating as empty.");
      return infs;
    }
  ...
  private static UUID resolveOwner(JSONObject user, int row) {
    ...
    Oneblock.plugin
        .getLogger()
        .warning("[Oneblock] PlData.json row " + row + " has neither 'uuid' nor 'nick'; skipping");
    return null;
  }
