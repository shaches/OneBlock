...
public final class ConfigManager {
  ...
  public void loadMainConfig() {
    ...
    if (Oneblock.config.isSet("custom_island") && !Compat.legacy) Island.read(Oneblock.config);
  ...
  private void DatabaseConfig() {
    ...
    DatabaseManager.autoReconnect =
        readOrDefault("database.autoReconnect", DatabaseManager.autoReconnect);
  }
  ...
  void parsePoolEntry(Object raw, Level level) {
    ...
    if (raw instanceof Map) {
      ...
      if (text.charAt(0) == '/') {
        kind = "command";
        payload = text;
      } else if (text.toUpperCase().startsWith("CHEST:")) {
        kind = "chest";
    ...
    switch (kind) {
      ...
      case "mob":
        EntityType et;
        try {
          et = EntityType.valueOf(payload.toString().toUpperCase());
        } catch (Exception e) {
          Oneblock.plugin
              .getLogger()
  ...
  File getFile(String name) {
    ...
    if (!file.exists()) Oneblock.plugin.saveResource(name, false);
    return file;
  }
