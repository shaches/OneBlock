...
/**
 * Factory + {@code toString()} coverage for {@link PoolEntry}, the tagged
 * union used by {@link Level}'s block pool. {@code Kind.MOB} is intentionally
 * absent because mobs live in a separate {@code WeightedPool<EntityType>}.
 */
class PoolEntryTest {
    ...
    @Test
    @DisplayName("lootTable(key): LOOT_TABLE kind, value is the key, toString includes 'loot_table: '")
    void lootTableFactory() {
        ...
        assertThat(e.toString())
                .startsWith("loot_table: ")
                .contains("chests/simple_dungeon");
    }
    ...
    @Test
    @DisplayName("Kind enum is closed: four cases, no MOB")
    void kindEnumShape() {
