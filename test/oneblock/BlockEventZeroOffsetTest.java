...
/**
 * Regression test for the divide-by-zero crash in {@link BlockEvent#ItemStackSpawn}
 * when the plugin has not yet been configured ({@code offset == 0}).
 */
class BlockEventZeroOffsetTest {
    ...
    @Test
    @DisplayName("ItemStackSpawn must not throw when offset is zero (unconfigured plugin)")
    void itemStackSpawnSurvivesZeroOffset() {
        ...
        assertThatCode(() -> handler.ItemStackSpawn(event)).doesNotThrowAnyException();
    }
}
