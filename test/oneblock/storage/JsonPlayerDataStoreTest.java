...
/**
 * Round-trip coverage for {@link JsonPlayerDataStore}, the JSON-backed
 * player data store on the steady-state save/load path. Phase 5.3
 * lifts this class from {@code 0%} line coverage; a regression here is
 * a data-loss class because every server's island roster lives in the
 * file this store writes.
 *
 * <p>The store's {@code f} field is a {@code static final File} that
 * eagerly evaluates {@code Oneblock.plugin.getDataFolder()} at
 * class-load time. To make that initialiser succeed in a unit test
 * (no real Bukkit server, no real plugin), {@link #installMockPlugin()}
 * stubs {@link Oneblock#plugin} with a Mockito mock whose
 * {@code getDataFolder()} returns a class-scoped {@link TempDir} path,
 * then forces the class to load via {@link Class#forName(String)}
 * <em>before</em> any test method runs. The static {@code f} is
 * subsequently a stable handle to {@code <tempDir>/PlData.json} for
 * the lifetime of this test class.
 *
 * <p>Each test deletes the on-disk JSON in {@link #cleanFile()} so
 * stale data from the previous test cannot leak into the next read.
 *
 * <p>The nick-fallback path in {@code resolveUuid} is intentionally
 * <b>not</b> covered here - it calls {@link oneblock.utils.Utils#getOfflinePlayerByName}
 * which dereferences {@link org.bukkit.Bukkit#getOfflinePlayers()}
 * and only works on a real server. UUID-keyed entries (the modern
 * primary path) cover ~80 % of the class.
 */
class JsonPlayerDataStoreTest {
    ...
    @Test
    @DisplayName("round-trip: a null-slot (uuid==null) entry writes as JSON null and reads back as the not_found-equivalent")
    ...
    void invatedTypoAcceptedOnRead() throws Exception {
        ...
        String json = "{\"id\":1,\"0\":{\"uuid\":\"" + owner + "\",\"lvl\":0,\"breaks\":0,"
                + "\"invated\":[\"" + invited + "\"]}}";
    ...
    void readNonNumericIdWarns() throws Exception {
        ...
        assertThat(msg.getAllValues())
            .anyMatch(s -> s.contains("missing or non-numeric 'id'"));
    }
    ...
    void rowWithoutUuidOrNickWarns() throws Exception {
        ...
        verify(pluginLogger, atLeastOnce()).warning(msg.capture());
        assertThat(msg.getAllValues())
            .anyMatch(s -> s.contains("neither 'uuid' nor 'nick'"));
    }

    @Test
    @DisplayName("read with a non-UUID-shaped invitee token falls through to the nick lookup, logs 'unresolved' on miss")
    void invalidInvitedUuidSkippedWithWarning() throws Exception {
        ...
        String json = "{\"id\":1,\"0\":{\"uuid\":\"" + owner + "\","
                + "\"invited\":[\"" + validInvitee + "\",\"not-a-uuid-at-all\"]}}";
        try (FileWriter w = new FileWriter(JsonPlayerDataStore.f)) {
            w.write(json);
        }
        ...
        verify(pluginLogger, atLeastOnce()).warning(msg.capture());
        assertThat(msg.getAllValues())
            .anyMatch(s -> s.contains("unresolved"));
    }

    @Test
    @DisplayName("read on a syntactically broken JSON file logs a warning and returns empty (no throw)")
